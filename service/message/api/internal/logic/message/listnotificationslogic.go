...
func (l *ListNotificationsLogic) ListNotifications(req *types.PageReq) (resp *types.NotificationListResp, err error) {
	started := time.Now()
	const route = "/message/v1/notifications/list"
	defer func() {
	...
	rpcResp, err := l.svcCtx.MessageRpc.ListNotifications(l.ctx, &pb.NotificationListReq{
		UserId:     uid,
		Offset:     req.Offset,
		Limit:      req.Limit,
	...
	return shared.ToNotificationListResp(rpcResp), nil
}
